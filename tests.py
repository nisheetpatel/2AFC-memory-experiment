...
from expt.info import get_config_info, set_file_path
...
def simulate_experiments() -> pd.DataFrame:
    ...
    for config_info in itertools.product(subject_ids, session_ids, session_types):
        ...
        # collect option data
        option_rewards = [option.meanReward for option in choice_options[:12]]
        option_type = [option.shape.name for option in choice_options[:12]]
